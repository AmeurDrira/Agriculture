...
void LORA_Send_Data(unsigned char *Data, unsigned char Data_Length, unsigned int Frame_Counter_Up)
{
  ...
  //Serial.println(Frame_Counter_Up);
  //Encrypt the data
  Encrypt_Payload(Data, Data_Length, Frame_Counter_Up, Direction, DevAddr);
...
unsigned char LORA_Receive_Data(unsigned char *Data)
{
  ...
  if (Message_Status == CRC_OK)
  {
    ...
    //Serial.print("Got data to device :");
    //Serial.print(RFM_Data[1]); Serial.print(RFM_Data[2]); Serial.print(RFM_Data[3]); Serial.println(RFM_Data[4]);
    //Serial.print("local mac address :");
    //Serial.print(DevAddr[0]); Serial.print(DevAddr[1]); Serial.print(DevAddr[2]); Serial.println(DevAddr[3]);

    if ( (RFM_Data[1] && 0xFE) == (DevAddr[3] && 0xFE))  {
      if (( ((RFM_Data[1] && 0x01) == (DevAddr[3] && 0x01)) && (RFM_Data[2] == DevAddr[2]) && (RFM_Data[3] == DevAddr[1]) && (RFM_Data[4] == DevAddr[0])) || (((RFM_Data[1] && 0x01 == 0x01) && (RFM_Data[2] == 0xFF)) && (RFM_Data[3] == 0xFF) && (RFM_Data[3] == 0xFF) && (RFM_Data[4] == 0xFF)) ) {

        //Serial.println("Adress match found");
  ...
  if (Message_Status == MIC_OK)
  {
    ...
    if (Data_Length != 0x00)
    {
      Encrypt_Payload(&RFM_Data[Data_Location], Data_Length, Frame_Counter_Down, Direction, RFM_Data + 1);
...
/**************************************************************************/
/**************************************************************************/
void LORA_announce(unsigned char *Data, unsigned char Data_Length, unsigned int Frame_Counter_Up)
{
  //Define variables
  unsigned char i;
  ...
  //A1
  unsigned char Mac_Header = 0xE0;

  unsigned char Frame_Control = 0x00;
  unsigned char Frame_Port = 0x01;
  //Serial.print("sending announcment : ");
  //Serial.println(Frame_Counter_Up);
  //Encrypt the data
  Encrypt_Payload(Data, Data_Length, Frame_Counter_Up, Direction, DevAddr);
